Shader "AP01/L05/OldShool_Phong" { /*路徑名稱*/
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                //準備向量 
                float3 nDir = normalize(i.nDirWS);                                              //模型法向量
                float3 lDir = normalize(_WorldSpaceLightPos0.xyz);                      //光方向 
                float3 vDir = normalize(_WorldSpaceCameraPos.xyz - i.posWS);    //視覺方向
                float3 vrDir = reflect(-vDir, nDir);                                               //視覺反射方向

                //準備點乘結果
                float nDotl = dot(nDir, lDir);     //nDir Dot lDir 
                float vrDotl = dot(vrDir, lDir);   //vrDir Dot lDir 
                
                //光照模型 
                float lembert = max(0.0, nDotl);                                        //蘭伯特 
                float Phong = pow(max(0.0, vrDotl), _SpecularPow);            //Phong

                float3 finalRGB = _MainCol * lembert + Phong;                  //結果顏色 
